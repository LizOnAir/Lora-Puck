...
I decided to make a CAD case for the Lora module, so it can stay outdoor during rain and function normally. I start by importing the pcb and the display then draft a square shape for the base. I also went back to Kicad to measure the dimension of the board to make sure it’s correct. After that, I simply extrude the square so it covers the whole pcb and display. I leave the display part open and it’s almost done. But it feels too simple and plain. So I was thinking of adding some wedges or something where a string can attach to it, making it easier to tie on a tree or hanging it somewhere. I changed a few version, on the side / top, and found that a simple hole on the top fit the best. I also make buttons with the B and R carve on it. 
